package com.sino.cg.meta.introspector;
...
public class DatabaseIntrospector {
	...
	static {
		...
		typeMap.put(Types.INTEGER, new JdbcTypeInformation("INTEGER", new FullyQualifiedJavaType(int.class.getName())));
	...
	protected DatabaseConfig getLocalDatabaseConfig(DatabaseConfig config) throws SQLException {
		...
		if (dbMetadataUtils.getDatabaseMetaData().storesLowerCaseIdentifiers()) {
			localCatalog = config.getCatalog() == null ? null
					: config.getCatalog()
							.toLowerCase();
			localSchema = config.getSchemaPattern() == null ? null
					: config.getSchemaPattern()
							.toLowerCase();
			localTableName = config.getTableNamePattern() == null ? null
					: config
	...
	protected List<IntrospectedTable> calculateIntrospectedTables(
		...
		for (Map.Entry<IntrospectedTable, List<IntrospectedColumn>> entry : columns
			...
			for (IntrospectedColumn introspectedColumn : entry.getValue()) {
				...
				introspectedColumn.setJavaProperty(Strings.lowerFirst(Strings.line2Hump(introspectedColumn.getName(), true)));
				// 处理注释
