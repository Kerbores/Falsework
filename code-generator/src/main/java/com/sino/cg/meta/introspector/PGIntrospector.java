...
public class PGIntrospector extends DatabaseIntrospector {
	...
	protected Map<String, String> getTableComments(DatabaseConfig config) throws SQLException {
		...
		try {
			PreparedStatement preparedStatement = dbMetadataUtils.getConnection().prepareStatement(
	...
	protected Map<String, Map<String, String>> getColumnComments(DatabaseConfig config) throws SQLException {
		...
		try {
			...
			sqlBuilder.append(
					"SELECT col_description(a.attrelid,a.attnum) as comments,a.attname as cname,c.relname as tname FROM pg_class as c,pg_attribute as a where a.attrelid = c.oid and a.attnum>0 and c.relname not like 'pg_%' and c.relname not like 'sql_%') as temp where comments is not null ");
